...
public class RealTimeStrategyServiceImpl implements RealTimeStrategyService {
     ...
     */
    public Map<String, Object> executeRealTimeBacktest(RealTimeStrategyEntity realTimeStrategy) {
        String strategyCode = (String) realTimeStrategy.getStrategyCode();
        String strategyName = (String) realTimeStrategy.getStrategyName();
        String symbol = (String) realTimeStrategy.getSymbol();
        String interval = (String) realTimeStrategy.getInterval();
        LocalDateTime startTime = realTimeStrategy.getStartTime();
        BigDecimal tradeAmount = BigDecimal.valueOf(realTimeStrategy.getTradeAmount());
        ...
        try {
            ...
            result = realTimeStrategyManager.startRealTimeStrategy(realTimeStrategy);

            if (result.get("status").equals(CANCELED)) {
                return result;
            }
            ...
            // 获取策略运行状态并设置Future
            RealTimeStrategyEntity runningState = realTimeStrategyManager.getRunningStrategy(strategyCode, symbol, interval);
